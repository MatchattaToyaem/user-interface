  ...
  >
      ...
      >
        <div v-if="isSidebarExpanded" class="brand">
          <div
            ...
            :class="{ receiveIntroLogo: showIntro }"
          >
        ...
        <TransitionGroup name="chat-delete" tag="div" class="history-list">
          ...
          >
            <div class="history-left">
              ...
              <span v-else class="history-text">{{ chat.name }}</span>
            </div>

            <div class="history-actions">
              <button class="history-menu" @click.stop="$emit('toggle-menu', chat.id)">
                •••
              </button>

              <div v-if="openMenuChatId === chat.id" class="history-dropdown">
                <button @click.stop="$emit('rename-chat', chat.id)">Rename</button>
                <button class="delete-action" @click.stop="$emit('delete-chat', chat.id)">
                  Delete Chat
                </button>
              </div>
            </div>
    ...
    <template v-else>
      ...
      <div class="collapsed-bottom">
        <button class="collapsed-logout-btn" @click="$emit('logout')" title="Logout">
          <span class="collapsed-logout-icon">↩</span>
          <span class="collapsed-tooltip">Logout</span>
        </button>

        <button class="collapsed-user-btn" @click="$emit('expand-sidebar')" :title="displayName">
...
.receiveIntroLogo {
  opacity: 0;
  animation: receiveMergedLogo 0.55s ease forwards;
